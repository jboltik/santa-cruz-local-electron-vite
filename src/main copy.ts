...
const createWindow = () => {
  ...
  // ✅ Register Cmd+F to trigger find-in-page
  // TODO REPLACE THIS WITH NON GLOBAL
  // globalShortcut.register('CommandOrControl+F', () => {
  //   if (mainWindow) {
  //     // mainWindow.webContents.findInPage('###');  start searching for ###
...
async function handleFileUpload() {
  ...
  try {
    ...
    if (htmlFile) {
      ...
      console.log('📢 Injecting into emailTemplate:');
...
function resolveTiplineHtml() {
  // Use the latest value if present, otherwise use what's in settings
  const raw = latestSignupPromptHtml ?? settingsCache.signupPromptHtml;

  if (raw === undefined) {
    return { enabled: true, html: DEFAULT_SIGNUP_PROMPT_HTML }; // never set ⇒ default
  }
  if ((raw ?? '').trim() === '') {
    return { enabled: false, html: '' }; // explicitly empty ⇒ disabled
  }
  return { enabled: true, html: raw! };  // custom
}
...
(() => {
  ...
  if (removed) {
    console.log(`🧹 Removed ${removed} empty headings (h2–h6).`);
  }
})();
...
const { enabled: tipEnabled, html: tipHtml } = resolveTiplineHtml();

if (tipEnabled) {
  const TIPLINE_START = '<!--TIPLINE_START-->';
  const TIPLINE_END   = '<!--TIPLINE_END-->';
  const wrappedTipline = `${TIPLINE_START}${tipHtml}${TIPLINE_END}`;

  const norm = (s: string) =>
    s.replace(/\s+/g, ' ').trim().toLowerCase().replace(/[.:–—-]+$/g, '');

  const $hotJobsH2 = $('h2').filter((_, el) => norm($(el).text()) === 'hot jobs').first();

  if ($hotJobsH2.length) {
    $hotJobsH2.before(wrappedTipline);
  } else {
    const $h2s = $('h2');
    if ($h2s.length >= 2) $h2s.eq(1).before(wrappedTipline);
    else if ($h2s.length === 1) $h2s.eq(0).after(wrappedTipline);
    else {
      const $firstH3 = $('h3').first();
      if ($firstH3.length) $firstH3.before(wrappedTipline);
      else $('body').prepend(wrappedTipline);
    }
  }
}
...
// if ($hotJobsH2.length) {
//   $hotJobsH2.before(signupPromptHtml);
// } else {
//   // Optional fallback: put it before the first H3 if "Hot jobs" is missing
//   const $firstH1 = $('h2').first();
//   if ($firstH1.length) $firstH1.after(signupPromptHtml);
// }
...
const H2_SNIPPET_HTML = `<p style="border: 1px solid #efefef;font-size: 0px;line-height: 0px;color: #202020;">&nbsp;</p>`;
...
// ----------------------------------------------
  ...
  let subjectFromH1 = $('h1').first().text().replace(/\s+/g, ' ').trim();
...
async function processAndValidateHtml(htmlContent: string) {
  ...
  // ✅ Send processed HTML & link validation results to renderer
  mainWindow?.webContents.send(
    'html-file-processed',
    processedHTML,
    previewText,
  ...
  // ✅ Fix: Ensure function returns the values
  return { processedHTML, previewText, linkResults, subjectFromH1 };
}

ipcMain.handle('signup-prompt:get', async () => {
  // undefined → never set → show default in editor
  if (settingsCache.signupPromptHtml === undefined) return DEFAULT_SIGNUP_PROMPT_HTML;
  // '' (empty) → disabled → editor shows empty
  return settingsCache.signupPromptHtml!;
});
